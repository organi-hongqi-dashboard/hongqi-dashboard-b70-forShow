 ...
 */
#define BoolValueChangeSet(name, v) \
    do { \
        if (m_##name != (v)) { \
            m_##name = (v); \
            emit name##Changed(v); \
...
#define NumValueChangeSet(name, v, min, max) \
    do { \
        if (qAbs(m_##name - qBound(min, v, max)) > 0.001) { \
            m_##name = qBound(min, v, max); \
            emit name##Changed(qBound(min, v, max)); \
        } \
    } while (0)

#define NumValueErrChangeSet(name, v, min, max, e) \
    do { \
        if (e == v) { \
            m_##name = (v); \
            emit name##Changed(v);\
